...
LONGITUDE_25 ,
LATITUDE_25 
FROM MIP.T_GEOS_25_DY g
WHERE LOG_DATE = to_date(:dateto, 'yyyymmdd') AND CL_NAME = :clname
) s 
ON ( s.LONGITUDE_25 = f.LONGITUDE_25 AND s.LATITUDE_25 = f.LATITUDE_25 AND s.CELLID_PCELL = f.CELLID_PCELL)
WHERE f.LOG_DATE = to_date(:datefrom, 'yyyymmdd') AND CL_NAME = :clname
UNION 
SELECT 
s.ENODEB_ID 
,s.CELLID_PCELL 
...
LATITUDE_25 
FROM MIP.T_GEOS_25_DY g 
WHERE LOG_DATE = to_date(:datefrom, 'yyyymmdd') AND CL_NAME = :clname
) f 
ON ( s.LONGITUDE_25 = f.LONGITUDE_25 AND s.LATITUDE_25 = f.LATITUDE_25 AND s.CELLID_PCELL = f.CELLID_PCELL)
WHERE s.LOG_DATE = to_date(:dateto, 'yyyymmdd') AND CL_NAME = :clname
...
,LONGITUDE_25 
,LATITUDE_25
FROM MIP.T_GEOS_25_DY 
WHERE LOG_DATE = to_date(:dateto, 'yyyymmdd') AND CL_NAME = :clname) a
